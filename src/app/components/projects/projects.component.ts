...
export class ProjectsComponent implements OnInit {
  ...
  filters = ['ALL', 'FULL STACK', 'MACHINE_LEARNING'];

  constructor(private svc: PortfolioService) { }

  ngOnInit() {
    this.svc.getProjects().subscribe(p => {
      this.projects = p;
      this.loading = false;
    });
  }
  ...
  filtered() {
    ...
    return this.projects.filter(
      p =>
        ...
        this.activeFilter.trim().toUpperCase()
    );
  }
  ...
  currentIndex() {
    return this.filtered().findIndex(p => p.id === this.selected?.id);
  }

  nextProject() {
    const i = this.currentIndex();
    if (i < this.filtered().length - 1) {
      this.selected = this.filtered()[i + 1];
    }
  }
